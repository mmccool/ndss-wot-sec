...
Nowadays the amount of different IoT devices around us is constantly increasing in all areas starting from smart home and ending up with industry automation. 
All these devices target different use cases, have different underlying implementations and connected using different communication protocols.
In this light a Web of Things (WoT)~\cite{Wot2017arch} aims to provide a single interoperability layer between IoT devices with a metadata defined in order to describe a generic networking interface exposed by an IoT device.
